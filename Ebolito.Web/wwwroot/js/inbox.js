(() => {
  ...
  function headers(json = false) {
    const value = { 'X-Ebolito-Profile-Admin-Key': adminKey.value.trim() };
  ...
  function render() {
    ...
    if (!visible.length) {
      inboxList.innerHTML = '<div class="empty">No engagements match this filter.</div>';
      return;
    }
    ...
    inboxList.innerHTML = visible.map(item => {
      ...
      return `<article class="inbox-card">
        <h2>${escapeHtml(customer.displayName || 'Verified customer')} <span class="status-pill">${escapeHtml(name)}</span></h2>
        <div class="inbox-meta"><span>${escapeHtml(e.location)}</span><span>Updated ${escapeHtml(formatDate(e.updatedAt))}</span><span>Delivered via ${escapeHtml(channelName(e.deliveredChannel))}</span></div>
        <p>${escapeHtml(e.requestText)}</p>
        <div class="inbox-meta"><span>${escapeHtml(customer.verifiedMobileNumber || '')}</span><span>${escapeHtml(customer.email || '')}</span></div>
        ${deliveries ? `<details class="delivery-list"><summary>Delivery history</summary><ul>${deliveries}</ul></details>` : ''}
        <div class="inbox-actions">${responseActions}${progressActions}${review}</div>
      </article>`;
    }).join('');
  ...
  async function load() {
    ...
    try {
      ...
      inboxList.innerHTML = '<div class="empty">Unable to load inbox.</div>';
    }
  }
  ...
  async function respond(id, responseValue) {
    ...
    try {
      await readResponse(await fetch(`/api/engagements/${encodeURIComponent(id)}/response`, { method: 'POST', headers: headers(true), body: JSON.stringify(responseValue) }));
      await load();
  ...
  async function progress(id, action) {
    ...
    try {
      await readResponse(await fetch(`/api/admin/engagements/${encodeURIComponent(id)}/progress`, { method: 'POST', headers: headers(true), body: JSON.stringify({ action }) }));
      await load();
    } catch (error) { globalStatus.textContent = error.message; }
  }
