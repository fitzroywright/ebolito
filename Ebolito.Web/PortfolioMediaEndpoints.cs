...
public static class PortfolioMediaEndpoints
{
    ...
    public static IEndpointRouteBuilder MapPortfolioMediaEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost("/api/admin/professionals/{id:guid}/portfolio-media", async (Guid id, HttpRequest request, IConfiguration configuration, CancellationToken ct) =>
        {
            ...
            var storageKey = $"ebolito/professionals/{id:D}/portfolio/{mediaId:N}{extension}";
            var stored = await storage.StoreAsync(new StorageWriteRequest(
                storageKey,
                upload,
                normalizedType,
                ...
                {
                    ["application"] = "Ebolito",
                    ["professionalId"] = id.ToString("D"),
                    ["purpose"] = "portfolio-image"
                },
                MaximumImageBytes), ct);
...
#else
        endpoints.MapGet("/api/admin/storage/capability", (HttpRequest request) =>
        {
            if (!ProfileAdministration.IsAuthorized(request)) return Results.Unauthorized();
            return Results.Ok(new { commonStorageCompiled = false, portfolioUploadAvailable = false });
        });
#endif
        return endpoints;
    }
    ...
    private static string? NormalizeImageContentType(string? contentType) => contentType?.Trim().ToLowerInvariant() switch
    {
