...
app.MapPost("/api/engineering/diagnostics/run", async (HttpRequest httpRequest, Ebolito.Web.EngineeringDiagnosticRunRequest request, EbolitoEngineeringDiagnostics diagnostics, CancellationToken ct) => { if (!EbolitoEngineeringDiagnostics.IsAuthorized(httpRequest)) return Results.Unauthorized(); return Results.Ok(await diagnostics.RunAsync(request, "Aegis.Diagnostics", ct)); });
